...
class Curvphormer(nn.Module):
    ...
    # ===== 修复后的动态曲率计算 =====
    def compute_dynamic_curvatures(self, h, edge_index, chunk_size=1024):
        """
        批量化动态曲率计算，适合GPU，控制显存
        修复：移除不必要的P张量存储，只关注Wasserstein距离计算
        """
        device = h.device
        num_edges = edge_index.size(1)
        ...
        # 构建邻居列表（静态一次性在CPU完成）
        neighbors = build_neighbor_list(edge_index, h.size(0), include_self=True)

        # 按chunk处理边
        for start in range(0, num_edges, chunk_size):
            end = min(start + chunk_size, num_edges)
            e_idx = slice(start, end)
            src_nodes = edge_index[0, e_idx]
            tgt_nodes = edge_index[1, e_idx]

            # 初始化Wasserstein距离存储
            wass_dist = torch.zeros(len(src_nodes), device=device)

            # 逐个处理每条边（避免批量padding导致的维度不匹配）
            for i in range(len(src_nodes)):
                src = src_nodes[i]
                tgt = tgt_nodes[i]

                # 获取源节点和目标节点的邻居
                src_nb = neighbors[src.item()]
                tgt_nb = neighbors[tgt.item()]
                
                # 提取邻居特征
                h_src = h[src_nb]  # [m, hidden_dim]
                h_tgt = h[tgt_nb]  # [n, hidden_dim]
                m, n = len(src_nb), len(tgt_nb)

                # 计算相似度分布 mu, nu
                sim_src = torch.matmul(h_src, h[src].unsqueeze(-1)).squeeze(-1)  # [m]
                sim_tgt = torch.matmul(h_tgt, h[tgt].unsqueeze(-1)).squeeze(-1)  # [n]
                mu = F.softmax(self.beta * sim_src, dim=0)  # [m]
                nu = F.softmax(self.beta * sim_tgt, dim=0)  # [n]

                # 计算代价矩阵 C (m x n)
                C = torch.cdist(h_src, h_tgt, p=2)  # [m, n]

                # Sinkhorn算法计算Wasserstein距离（只关注距离，不存储P）
                _, wass_dist_i = sinkhorn(
                    mu, nu, C,
                ...
                # 边长度 d_ij
                d_ij = torch.norm(h[src] - h[tgt], dim=0).clamp(min=1e-6)
                
                # 计算曲率值
                curvature_values[e_idx][i] = 1 - wass_dist_i / d_ij

        # 曲率编码器
        curvature_embeddings = self.curvature_encoder(curvature_values)  # [num_edges, hidden_dim]
